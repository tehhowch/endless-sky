...
TARGET="$HOME/$1"
cp tests/test_fleet_enter/main.cpp source/main.cpp
scons -j8 mode=profile && ./endless-sky
gprof endless-sky > "$TARGET".txt

ROOT_NODE="Fleet::Enter(System const&, std::__cxx11::list<std::shared_ptr<Ship>, std::allocator<std::shared_ptr<Ship> > >&, Planet const*) const"
